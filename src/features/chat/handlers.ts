  ...
  try {
    while (true) {
      ...
      while (receivedMessage.length > 0) {
        ...
        if (sentenceMatch || done) {
          var sentence = sentenceMatch?.[0] || ''
          if (sentenceMatch == null && done) {
            sentence = receivedMessage
          }
          // 区切った文字をsentencesに追加
          sentences.push(sentence)
 ...
 * @param content 話す内容
 */
export const processSpeakContent = async (content: string) => {
  await cancelSpeakCharacterCurrentEmotion()
  console.log('processSpeakContent start content=', content)
  const ss = settingsStore.getState()
  const hs = homeStore.getState()
  const currentSlideMessages: string[] = []
  ...
  while (receivedMessage.length > 0) {
    ...
    const aiTalks = textsToTalkNoEmotions([aiText], ss.koeiroParam)
    aiTextLog.push({ role: 'assistant', content: sentence })
  ...
  // TODO: ここにチャットログを後でRobotVRM用に考える
  // homeStore.setState({
  //   chatLog: [...currentChatLog, ...aiTextLog],
  //   chatProcessing: false,
  // })
